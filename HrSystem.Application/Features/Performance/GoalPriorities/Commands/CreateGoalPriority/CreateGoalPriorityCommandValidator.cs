...
public class CreateGoalPriorityCommandValidator : AbstractValidator<CreateGoalPriorityCommand>
{
    private readonly ApplicationDbContext _context;

    public CreateGoalPriorityCommandValidator(ApplicationDbContext context)
    {
        _context = context;
    ...
    {
        return !await _context.GoalPriorities.AnyAsync(gp => gp.NameAr == nameAr, cancellationToken);
    }

    private async Task<bool> BeUniqueNameEn(string nameEn, CancellationToken cancellationToken)
    {
        return !await _context.GoalPriorities.AnyAsync(gp => gp.NameEn == nameEn, cancellationToken);
    }
}
