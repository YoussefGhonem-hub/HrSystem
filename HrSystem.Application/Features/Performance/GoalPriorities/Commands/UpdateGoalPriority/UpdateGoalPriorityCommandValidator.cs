...
public class UpdateGoalPriorityCommandValidator : AbstractValidator<UpdateGoalPriorityCommand>
{
    private readonly ApplicationDbContext _context;

    public UpdateGoalPriorityCommandValidator(ApplicationDbContext context)
    {
        _context = context;
    ...
    private async Task<bool> BeUniqueNameAr(UpdateGoalPriorityCommand command, string nameAr, CancellationToken cancellationToken)
    {
        return !await _context.GoalPriorities.AnyAsync(gp => gp.NameAr == nameAr && gp.Id != command.Id, cancellationToken);
    }

    private async Task<bool> BeUniqueNameEn(UpdateGoalPriorityCommand command, string nameEn, CancellationToken cancellationToken)
    {
        return !await _context.GoalPriorities.AnyAsync(gp => gp.NameEn == nameEn && gp.Id != command.Id, cancellationToken);
    }
}
